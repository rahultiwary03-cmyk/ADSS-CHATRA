...
def load_data():
    try:
        ...
        # 💡 सबसे बड़ा फिक्स: कॉलम के नाम में छुपे 'Enter' (Newlines) और एक्स्ट्रा स्पेस को हटाना
...
if st.button("डाटा खोजें (Search)", type="primary"):
    if aadhar_input and df is not None:
        # आधार कॉलम को स्मार्ट तरीके से ढूंढना
        target_col = next((c for c in df.columns if 'aadhaar' in c.lower() or 'aadhar' in c.lower()), None)
        ...
        if target_col:
            ...
            if not match.empty:
                ...
                # 💡 स्मार्ट डेटा फ़ेचर (यह किसी भी स्पेलिंग मिस्टेक या एक्स्ट्रा स्पेस को इग्नोर कर देगा)
                def get_val(keywords, exclude=None):
                    for col in df.columns:
                        cl = col.lower()
                        if any(k in cl for k in keywords):
                            if exclude and any(e in cl for e in exclude): continue
                            val = res[col]
                            return str(val).strip() if pd.notna(val) and str(val).strip() != '' else "N/A"
                    return "N/A"

                # सभी कॉलम का डेटा सुरक्षित तरीके से निकालना
                name = get_val(['applicant', 'beneficiary', 'name'], exclude=['father', 'husband', 'bank'])
                father = get_val(['father', 'husband'])
                district = get_val(['district'])
                category = get_val(['category'])
                bank = get_val(['bank'])
                amount = get_val(['amount'])
                date = get_val(['sanctiondate', 'date'])
                status_val = get_val(['payment status', 'status']).upper()

                # स्टेटस के अनुसार बटन का रंग
                ...
                <div class="result-card">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 40px;">
                        <div style="flex: 1; min-width: 300px;">
                            ...
                            <div class="detail-label">📍 ज़िला / केटेगरी</div>
                            <div class="detail-value">{district} / {category}</div>
                        </div>
                        <div style="flex: 1; min-width: 300px;">
